...
export default function Columns({ boardData, setBoardData, isFormVisible, handleAddTaskBtn, ontextChange }) {
  const { setNodeRef } = useDroppable({ id: 'dropId', data: boardData });
    ...
    <div className='grid grid-cols-3 gap-5 my-5'  >
      {
        boardData?.map((board: any, index: any) => (
          <SortableContext strategy={verticalListSortingStrategy} key={index} id={board?._id} items={boardData}>
            <div ref={setNodeRef} key={index} className={`bg-gray-100 p-3 rounded-md shadow-md flex flex-col relative overflow-hidden`} >
